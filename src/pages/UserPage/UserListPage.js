...
export default function USERListPage() {
  ...
  return (
    <>
      <Helmet>
        <title> Danh Sách Người Dùng | BeeCine </title>
      </Helmet>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredList.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { id, name, role, email, avatarUrl, points, status } = row;
                    ...
                    return (
                      <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={selectedList}>
                        ...
                        <TableCell align="left">
                          <Label color={(role === 'premium' && 'warning') || 'info'}>{sentenceCase(role)}</Label>
                        </TableCell>
